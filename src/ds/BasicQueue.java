...
public class BasicQueue<X> {
    ...
    public X deQueue(){
        ...
        //otherwise if this is the last item on the queue, the queue needs to reset to empty
        else if(front == end){
            item = data[front];
            front = -1;
            end = -1;
    ...
    public X access(int position){
        if(size() == 0 || position > size()){
            throw new IllegalStateException("No items in the queue or the position is greater");
        }
